...
Every example on this page is playing: each player below was compiled from the file
under its “Source”, during this site's build.

<Cast name="basic" title="A whole file: examples/basic.terminal.yaml" />
...
<Cast name="keys" title="type, key and wait — with seeded jitter" />
...
<Cast name="output-delay" title="delay: output one line at a time" />
...
<Cast name="show" title="show: a file, highlighted at build time" />
...
<Cast name="recorded" title="exec:, after --record" lead="This was an exec: step. --record ran it once and replaced it with the command and exactly what it printed." />
...
<Cast name="colours" title="Markup, compiled" />
...
<Cast name="error-fix" title="An error, and its fix" lead="Real castwright validate output, recorded: the position, the known keys, a caret under the typo." />
